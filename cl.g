...
dec_bloc: (PROCEDURE^ proc_code dec_vars l_dec_blocs l_instrs ENDPROCEDURE! |
           FUNCTION^ func_code dec_vars l_dec_blocs l_instrs RETURN! expression ENDFUNCTION!)<</*needs modification*/ >>;

func_code: IDENT^ OPENPAR! l_param CLOSEPAR! RETURN! constr_type ;

proc_code: IDENT^ OPENPAR! l_param CLOSEPAR!  ;

l_param:  (dec_param (COMMA! dec_param)* |)	<<#0=createASTlist(_sibling);>>;

dec_param: (VAL^ | REF^) IDENT constr_type  ;

constr_type: BOOL | INT | STRUCT^ (field)* ENDSTRUCT! | ARRAY^ OPENKEY! INTCONST CLOSEKEY! OF! constr_type ;

field: IDENT^ constr_type;

l_instrs: (instruction)* <<#0=createASTlist(_sibling);>>;

l_values: (expression (COMMA! expression)* | )  <<#0=createASTlist(_sibling);>>;

instruction:
      whiledo | ifthenelse | IDENT ((DOT^ IDENT | OPENKEY^ expression CLOSEKEY!)* ASIG^ expression | OPENPAR^ l_values CLOSEPAR!)
      |	(WRITE^ | WRITELN^) OPENPAR! ( expression | STRING ) CLOSEPAR! | READ^ OPENPAR! IDENT CLOSEPAR! | OPENANGL^ l_values CLOSEANGL! ASIG^ expression;

ifthenelse: IF^ expression THEN! l_instrs (ELSE! l_instrs | ) ENDIF!;

whiledo: WHILE^ expression DO! l_instrs ENDWHILE!;

expression: expression1 (OR^ expression1 | AND^ expression1)*;

expression1: expression2 (EQ^ expression2 | GT^ expression2 | LT^ expression2 | );

expression2: expression3 (PLUS^ expression3 | MINUS^ expression3)*;

expression3: expression4 (TIMES^ expression4 | DIV^ expression4)*;

expression4: NOT^ expression4 | MINUS^ expression4 | expression5 ;

expression5: OPENPAR! expression CLOSEPAR! | expsimple ;

expsimple:
        IDENT^ (DOT^ IDENT | OPENKEY^ expression CLOSEKEY! | OPENPAR^ l_values CLOSEPAR!)*
      | INTCONST | CIERTO | FALSO | OPENANGL^ l_values CLOSEANGL! 
      ;